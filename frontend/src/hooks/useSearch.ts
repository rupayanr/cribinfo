...
function generateResponseText(
  ...
  if (matchType === 'partial' && relaxedDescriptions.length > 0) {
    const relaxedText = relaxedDescriptions.join(' ')
    ...
    return `I couldn't find exact matches for ${relaxedText}, but here are ${count} similar properties:`
  }
...
export function useSearch() {
  ...
  }, [query, city, setResults, setLoading, setError])
  ...
  const searchWithChat = useCallback(async (searchQuery: string, searchCity?: string) => {
    ...
    try {
      ...
      addAssistantMessage(
        'error',
        `Sorry, I encountered an error while searching: ${errorMessage}. Please try again.`
      )
    } finally {
      setTyping(false)
    }
  }, [city, addUserMessage, addAssistantMessage, setTyping])
